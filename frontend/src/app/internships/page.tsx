...
export default function InternshipsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      else setError(allData.message ?? 'Failed to load internships.');
      ...
      <main className={styles.main}>
        ...
        <section className={styles.hero}>
          <div className="container">
            <h1 className={styles.heroTitle}>Internships</h1>
            <p className={styles.heroSub}>Build practical skills through guided learning, real-world projects and hands-on experience.</p>
          </div>
        </section>
        ...
        <div className="container">
          ...
          {!loading && trending.length > 0 && (
            <TrendingCarousel items={trending} type={TYPE} title="Trending Internships" />
          )}
          ...
          <div className={styles.contentArea}>
            ...
            <div className={styles.grid}>
              <div className={styles.gridHeader}>
                <h2 className={styles.gridTitle}>All Internships</h2>
              ...
              {!loading && filtered.length === 0 && !error && (
                <div className={styles.state}>
                  <p className={styles.emptyMsg}>
                    {all.length === 0
                      ? 'No internships available yet. Add content in the admin panel.'
                      : 'No internships match your filters.'}
                  </p>
                  {all.length > 0 && (
                    <button className={styles.clearBtn} onClick={() => setFilters(defaultFilters)}>
                      Clear Filters
                    </button>
                  )}
                </div>
              ...
              <div className={styles.cardGrid}>
                {filtered.map((item) => (
                  <CatalogCard key={item._id} item={item} type={TYPE} />
                ))}
              </div>
            </div>
          </div>
